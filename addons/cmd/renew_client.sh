...
if [ -f "$RSA_DIR/pki/index.txt" ] && grep -qP "CN=${USERNAME}(\s|$)" "$RSA_DIR/pki/index.txt"; then
    log "User $USERNAME exists, renewing..."
    cd "$RSA_DIR" || exit 1

    if ./easyrsa --batch renew "$USERNAME" && ./easyrsa --batch gen-crl; then
        log "User $USERNAME renewed successfully"
        ...
        log "Failed to renew user $USERNAME"
        exit 1
    fi
else
